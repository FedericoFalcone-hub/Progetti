...
function getProdottiOrdinabili()
{
    ...
    $sql = "SELECT *
            ...
            where prezzo_medio IS NOT NULL
...
function aggiornaUtente($cf,$old_mail, $mail, $nome, $cognome, $telefono)
{
    $db = open_pg_connection();

    $sql = "SELECT comicgalaxy.aggiorna_utente($1, $2, $3, $4, $5, $6)"; 
    $params = array($cf,$old_mail, $mail, $nome, $cognome, $telefono);
    ...
    if (!$result) {
        ...
        if (str_contains($result, '16 caratteri')) {
            close_pg_connection($db);
            return 'ERRORE: Codice fiscale non valido (deve esssere 16 caratteri)';
        }

        if (str_contains($result, 'value too long')) {
            close_pg_connection($db);
            return 'ERRORE: Codice fiscale non valido (oltre 16 caratteri)';
        }

        if (str_contains($result, 'formato')) {
            close_pg_connection($db);
            return 'ERRORE: Codice fiscale non valido (formato errato)';
        }

        if (str_contains($result, 'utente_pkey')) {
            close_pg_connection($db);
            return 'ERRORE: Email già in uso';
        }

        if (str_contains($result, 'Numero')) {
            close_pg_connection($db);
            return 'ERRORE: formato numero di telefono non valido, deve contenere un prefisso e solo numeri.';
        }
...
function getClienti()
{
    ...
    $result = pg_prepare($db, "get_clienti", $sql);
    $result = pg_execute($db, "get_clienti", array());
    $utenti = array();

    while ($row = pg_fetch_assoc($result)) {
        if ($row['sospeso_tessera']==NULL){
            $stato_tessera="Nessuna";
        }
        else if ($row['sospeso_tessera']==='t'){
            $stato_tessera='Sospesa';
        } else if ($row['data_scadenza']< date('Y-m-d')){
            $stato_tessera='Scaduta';
        } else {
            $stato_tessera='Attiva';
        }
        $utenti[] = array(
            'mail' => $row['mail'],
            'nome' => $row['nome'],
...
function getNegozi()
{
    ...
    $sql = "SELECT n.telefono, n.id, n.nome, n.data_chiusura, i.citta, i.via, i.civico, m.nome AS nome_manager, m.cognome AS cognome_manager
...
function getProdottiFornitore($p_iva)
{
    ...
    $sql = "SELECT p.id, p.nome, f.prezzo, f.quantita
            ...
            WHERE p_iva_fornitore = $1 and f.prezzo IS NOT NULL
            ORDER BY nome";
...
function aggiorna_fornitore($old_p_iva, $p_iva, $nome, $telefono, $mail, $via, $civico, $citta)
{
    ...
    if (!$result) {
        ...
        if (str_contains($error, 'fornitore_pkey')) {
            close_pg_connection($db);
            return 'ERRORE: Partita IVA già in uso';
        }
        
        if (str_contains($error, 'formato')) {
            close_pg_connection($db);
            return 'ERRORE: Formato Partita IVA non valido (deve contenere esattamente 11 cifre)';
        }

        if (str_contains($error, 'Numero')) {
            close_pg_connection($db);
            return 'ERRORE: Formato telefono non valido (deve contenere un prefisso e solo numeri)';
        }

        if (str_contains($error, 'fornitore_mail_unique')) {
            close_pg_connection($db);
            return 'ERRORE: Email già in uso';
        }

        if (strpos($result, '23505') !== false || strpos($result, 'indirizzo_univoco') !== false) {
           return  "ERRORE: questo indirizzo è già associato ad un fornitore o negozio";
        }
...
function aggiorna_negozio($id_negozio, $nome, $telefono, $citta, $via, $civico)
{
    ...
    if (!$result) {
        $error = pg_last_error($db);
        if (str_contains($error, 'Numero di telefono')) {
            close_pg_connection($db);
            return 'ERRORE: formato numero di telefono non valido, deve contenere un prefisso e solo numeri.';
        }else if (strpos($result, '23505') !== false || strpos($result, 'indirizzo_univoco') !== false) {
           return  "ERROREA: questo indirizzo è già associato ad un fornitore o negozio";
            } else {
            close_pg_connection($db);
...
function getProdottiNegozio($id_negozio)
{
    ...
    $sql = "SELECT p.id, p.nome, p.descrizione, n.quantita, n.prezzo
            ...
            WHERE n.id_negozio = $1 AND n.prezzo IS NOT NULL
            ORDER BY p.nome";
...
function getDettaglioAcquisto($id_fattura)
{
   ...
   FROM comicgalaxy.dettaglio_fattura df
     ...
     JOIN comicgalaxy.fattura f ON df.id_fattura = f.id
   WHERE df.id_fattura = $1";
...
function getDisponibilitaProdotto($id) {
    $db = open_pg_connection();

    $sql = "SELECT *, n.nome as nome_negozio
            FROM comicgalaxy.prodotto p INNER JOIN comicgalaxy.fornitura_negozio f ON p.id = f.id_prodotto INNER JOIN comicgalaxy.negozio n ON f.id_negozio = n.id
            inner join comicgalaxy.indirizzo i on i.id=n.id_indirizzo 
            WHERE id_prodotto = $1 and prezzo is not null";

    $param = array($id);
    $result = pg_prepare($db, "get_disponibilita_prodotto", $sql);
    $result = pg_execute($db, "get_disponibilita_prodotto", $param);
...
function rimuoviVenditaProdotto($id_negozio, $id_prodotto) {
    $db = open_pg_connection();

    $sql = "UPDATE comicgalaxy.fornitura_negozio
            SET prezzo = NULL
            WHERE id_prodotto = $1 AND id_negozio = $2";

    $params = array($id_prodotto, $id_negozio);
    $result = pg_prepare($db, "rimuovi_vendita_prodotto", $sql);
    $result = pg_execute($db, "rimuovi_vendita_prodotto", $params);

    close_pg_connection($db);
    return $result;
}

function getCFByMail($mail)
{
    $db = open_pg_connection();
    ...
    $param = array($mail);
    $result = pg_prepare($db, "get_cf_by_mail", $sql);
    $result = pg_execute($db, "get_cf_by_mail", $param);
...
function aggiornaCF($mail, $cf){
    $db = open_pg_connection();

    $sql = "UPDATE comicgalaxy.cliente
            SET cf = $1
            WHERE mail = $2";

    $params = array($cf, $mail);
    $result = pg_prepare($db, "aggiorna_cf", $sql);
    $result = pg_execute($db, "aggiorna_cf", $params);

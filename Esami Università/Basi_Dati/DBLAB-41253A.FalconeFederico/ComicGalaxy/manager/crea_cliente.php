...
if (isset($_POST['crea'])) {
    ...
    if ($result === true) {
        ...
        if (str_contains($result, $mail)) {
            $error_msg = "Errore: la mail inserita è già presente nel sistema.";
        } else if (str_contains($result, $cf)) {
            $error_msg = "Errore: il codice fiscale inserito è già presente nel sistema.";
        } else if (str_contains($result, 'telefono')) {
            $error_msg = "Errore: Numero di telefono non valido: deve contenere prefisso e solo cifre (es. +391234567890)";
        } else {
            $error_msg = "Errore nella creazione dell'utente: " . $result;
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Crea Cliente - ComicGalaxy</title>

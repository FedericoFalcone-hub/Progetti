...
if (isset($_POST['crea_fornitore'])) {
    ...
    if ($result === true) {
        ...
        }else if (strpos($result, 'telefono')){
            $_SESSION["error"] = "Errore: il numero di telefono inserito non è valido (deve contenere il prefisso).";
        }else if (strpos($result, 'fornitore_pkey')){
            $_SESSION["error"] = "Errore: questa partita IVA è già associata ad un fornitore.";
        }
     
        else {
            $_SESSION["error"] = "Errore nella creazione del fornitore: " . $result;
        }
    }
    $fornitori = getFornitori();
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        ...
                                        <form action="gestione_fornitori.php" method="POST" style="display:inline;">
                                            <input type="hidden" name="p_iva" value="<?=$f["p_iva"]?>">

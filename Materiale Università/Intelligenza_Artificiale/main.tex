...
Una \textbf{funzione di utilità} di un agente è un'internalizzazione della misura di prestazione. Purchè la funzione di utilità interna e la misura di presazione esterna concordino, un agente che sceglie le azioni per massimizzare l'utilità sarà razionale in base alla misura di prestazione esterna.
...
Nell'albero di ricerca, teniamo traccia del nostro accumulato sul percorso dal nodo iniziale a ogni noto $V$: $g(V)$. Non consideriamo EQL. \\
...
\\ Per adesso assumiamo che trovare la strategia ottima sia un problema di search che tiene conto dell'avverario: \textbf{aversarial search}. L'approccio di base con cui si risolve la classe di giochi che stiamo considerando è dato dall'algormitmo \textbf{\textcolor{red}{minimax}}, sostanzialmente una DFS sull'albero di gioco con un \textbf{ritorno all'indietro} dei valori di utilità.
...
\begin{itemize}
    ...
    \item Sì: on-policy, l'apprendimento considera esplicitamente l'esporazione.
\end{itemize}
\subsection{Regret}

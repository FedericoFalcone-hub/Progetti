...
\\ Anzichè guardare avanti ottimisticamente guardiamo avanti \textbf{realisitcamente}: si sceglie l'azione che effettivamente verrà scelta nel prossimo step, che chiamiamo $a'$.
...
\begin{itemize}
    \item No: off-policy, nell'apprendimento non viene considerato il cossto dell'esplorazione.
    \item Sì: on-policy, l'apprendimento considera esplicitamente l'esporazione.
\end{itemize}
\subsection{Regret}
Con gli approcci visti fino ad ora l'agente cerca di imparare la policy ottima attraverso ripetute interazioni con l'ambiente. Durante questa fase di apprendimento si commettono degli errori: regret, al differenza tra ciò 
che mi aspetto di ottenre con un algoritmo di apprendimento e ciò che avrei ottenuto eseguendo la policy ottima fin dall'inizio. È un costo pagato dall'agente per colmare il gap tra non conoscere nulla e conoscere la policy ottima,
...
\begin{itemize}
    \item \textcolor{blue}{Greedy}: troppa esploitation, regret cresce linearmente;
...
Dove $f_1(s,a), f_2(s,a), \dots, f_n(s,a)$ sono delle \textbf{feature} che descrivono la coppia stato-azione $(s,a)$ e $\Theta_0, \Theta_1, \dots, \Theta_n$ sono feature della coppia stato azione, che possiamo calcolare
...
Prima la stima di $Q(s,a)$ era una grande tabella con un valore per ogni coppia stato azione, la funzione era rappresentata in modo \textbf{esplicito}. Ora ci basta un vettore di parametri \textbf{$\theta$} $= \langle \theta_0, \theta_1, \dots, \theta_k \rangle$, la funzione è 
rappresentata in modo implicito, dato \textbf{$\theta$} e le feature di una coppia stato-azione calcoliamo $Q(s,a)$. Come scegliere \textbf{$\theta$}? \textcolor{red}{Dobbiamo pensare al problema in termini diversi}.
...
\end{figure}
Assunta una funzione di valutazione lineare con $k$ feature, rappresentata dal vettore di parametri $\theta$.
\\ Dato il dataset $\mathbb{T}$ (ottenuto con esplorazione)
\\ \textcolor{red}{\textbf{Problema}}: come calcolare $\theta$ in modo che possa valutare vettori di feature $x_{new}$ che descrivano coppie stato-azione mai viste prima e senza esplorarle?
...
\qquad \textcolor{ForestGreen}{\text{gree si noti che } x_0=1}
\]
...
$\mathcal{H}$ misuta l'incertezza (il disordine in termini di informazione) nella variabile aleatoria $V$. 
\begin{itemize}
...
Il metodo non parametrico più semplice possibile si chiama \textbf{tabella lookup}. 
...
Supponiamo di aver eseguito il nostro algoritmo su un training set e di aver appreso un modello/ipotesi (ad esempio i parametri di una regressione). Quanti errori fa il modell se applicato allo stesso training set?
